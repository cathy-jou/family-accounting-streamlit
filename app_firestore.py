    ...
    if df_filtered.empty:
        ...
        for index, row in df_filtered.iterrows():
            ...
            if record_id == st.session_state.get('editing_record_id'):
                ...
                # --- 模式 A：顯示「編輯表單」 ---
                with st.form(key=f"edit_form_{record_id}", clear_on_submit=False):  # 📌 表單開始（強化版）
                    ...
                    with edit_cols_1[2]:
                        ...
                        new_amount = st.number_input("金額", min_value=0, value=int(record_amount) if record_amount is not None else 0, step=1, format="%d", key=f"edit_amount_{record_id}")
                    ...
                    # 送出/取消按鈕（永遠在表單最後，以避免 Missing Submit Button）

                    form_cols = st.columns([1, 1, 3])

                    save = form_cols[0].form_submit_button("💾 儲存變更", use_container_width=True, type="primary")

                    cancel = form_cols[1].form_submit_button("❌ 取消", use_container_width=True, type="secondary")
