...
# 定義交易類別 (已修改為簡化版本：食衣住行育樂)
CATEGORIES = {
    '收入': ['薪資', '投資收益', '禮金', '其他收入'],
    # 簡化支出類別為 食, 衣, 住, 行, 育, 樂 (六大類)
    '支出': ['食', '衣', '住', '行', '育', '樂']
}
...
# 修正 #4: 移除銀行帳戶相關邏輯，將類別選擇從子類別簡化為直接選擇 (使用新的 CATEGORIES)
...
# 修正 #7: 移除設定餘額頁面中所有關於銀行帳戶的操作，僅保留總餘額設置提示
